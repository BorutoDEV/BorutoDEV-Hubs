...
local TabHandles = {
    ...
    Bring = Tabs.Main:Tab({ Title = "Bring Items", Icon = "layout-grid", Desc = "" }),
    ...
    Create = Tabs.Play:Tab({ Title = "Auto", Icon = "layout-grid", Desc = "" }),
...
-- ===== AUTO BRING SECTION =====
TabHandles.Bring:Paragraph({ Title = "Auto Bring Items" })

-- Item Categories (EDIT THESE TO MATCH YOUR GAME'S ITEM NAMES)
local itemCategories = {
    ["Weapons"] = {"Revolver", "Rifle", "Revolver Ammo", "Rifle Ammo", "Shotgun", "Bow"},
    ["Medicine"] = {"Bandage", "MedKit", "Pills", "First Aid Kit"},
    ["Food"] = {"Cake", "Cooked Steak", "Cooked Morsel", "Steak", "Morsel", "Berry", "Carrot", "Apple", "Can of Beans", "Soda"},
    ["Armor"] = {"Leather Body", "Iron Body", "Helmet", "Vest", "Military Helmet"},
    ["Scrap"] = {"Tyre", "Bolt", "Broken Fan", "Broken Microwave", "Sheet Metal", "Old Radio", "Washing Machine", "Old Car Engine", "Scrap Metal"},
    ["Fuel"] = {"Log", "Chair", "Coal", "Fuel Canister", "Oil Barrel", "Biofuel", "Wood", "Plank"},
    ["Tools"] = {"Axe", "Pickaxe", "Hammer", "Wrench", "Hatchet", "Shovel"},
    ["Materials"] = {"Cloth", "Rope", "Nails", "Duct Tape", "Glass", "Brick"},
}

-- Function to bring items to player
local function bringItemsToPlayer(itemNames)
    ...
    for _, itemName in ipairs(itemNames) do
        for _, item in ipairs(workspace:GetDescendants()) do
            if item.Name == itemName and (item:IsA("BasePart") or item:IsA("Model")) then
                ...
                if part then
                    pcall(function()
                        part.CFrame = hrp.CFrame * CFrame.new(0, 0, -5) -- Spawn 5 studs in front
                        if item:IsA("Model") then
                            part.Anchored = false -- Unanchor if needed
                        end
                    end)
                end
            end
...
for categoryName, items in pairs(itemCategories) do
    TabHandles.Bring:Button({
        Title = "Bring All " .. categoryName,
        Callback = function()
            bringItemsToPlayer(items)
        end
    })
end

-- ===== END OF AUTO BRING SECTION =====
...
TabHandles.Esp:Paragraph({ Title = "Esp NPC" })
...
TabHandles.Esp:Paragraph({ Title = "Item" })
